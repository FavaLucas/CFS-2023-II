...
class BinanceTrx {
  ...
  constructor(binanceID: string, destinationID: string, amount: number, trxDate: string) {
...
class BinanceProcesadorTrxAdapter {
  ...
  almacenarTrxBtn(trx: BinanceTrx): void {
    ...
    const thebitTrx = new BitcointTrxDateable(trx.getBinanceID(), trx.getDestinationID(), trx.getAmount(), fechaDate);
...
const trxBinance = new BinanceTrx('fdsfsdfe4343', 'ghdhhfjkdskhfjdy7333gfdsj', 0.0012, '2023-09-05');
